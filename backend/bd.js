...
class banco {
    ...
    conectar(){
        try{
            this.conn = new bd.Pool({ host:this.host, user:this.username, password:this.password, database:this.db_name, port: this.port, ssl:true });
        }
        catch (exception){
        ...
        try{
            ...
            this.conn.query(this.sql, this.params, function(err, result){
                ...
                if (err) 
                    ...
                    self.rowcount = result.rows.length;
